...
trait Sql_DB
{
    ...
    {
        $query=$this->Sql_DB_Create_Query($db);
        $this->DB_Query($query);
    }
    ...
    function Sql_DB_Exists($dbhash=array())
    {
        //Do not use method DBHash method here!
        //Causes recursive calling.
        if (empty($dbhash)) { $dbhash=$this->DBHash; }

        $dbs=$this->Sql_DBs_Get();

        $res=FALSE;
        if (preg_grep('/^'.$dbhash[ "DB" ].'$/',$this->Sql_DBs_Get())) { $res=TRUE; }
    ...
    function Sql_DB_String()
    {
        $dbstring="";
        if ($this->DB_Dialect()=="mysql")
        {
            $dbstring=
