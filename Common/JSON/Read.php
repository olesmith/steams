...
trait JSON_Read
{
    ...
    function JSON_Read_Items_Page($module,$datas,$where,$offset)
    {
        ...
        {
            $this->ApplicationObj()->MyApp_Interface_Message_Add("JSON_Read_Items: output error");
            $query=
                $this->JSON_Query_Pack
                (
                    $this->JSON_Query_Module_Datas($module,$datas,$offset,$where)
    ...
    function JSON_Read_Item($module,$datas,$sigaa_id)
    {
        if (empty($this->JSON_Items[ $sigaa_id ]))
        {
            ...
            if (empty($item))
            {
                $this->ApplicationObj()->MyApp_Interface_Message_Add("Warning! SIGAA item '".$sigaa_id."' not found!!!");
                #var_dump($items);
            }
            
            if ($found>1)
            {
                $this->ApplicationObj()->MyApp_Interface_Message_Add("Warning! Non-unique SIGAA item!!!");
                #var_dump($items);
            }
