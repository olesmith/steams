...
trait MyMod_Search
{
    ...
    function MyMod_Search_Items($searchvars=array(),$novals=0,$postvars=TRUE)
    {
        ...
        foreach ($this->ItemHashes as $id => $item)
        {
            ...
            foreach ($searchvars as $data => $value)
            {
                ...
                if (preg_match('/\S/',$value))
                {
                    ...
                    else
                    {
                        ...
                        if ($novals!=0)
                        {
                            if (!preg_match('/^'.$svalue.'$/i',$itemvalue))
                            {
                                $include=0;
                                $includedata=$data;
                                $includevalue=$svalue;
                        ...
                        elseif (!preg_match('/'.$svalue.'/i',$itemvalue))
                        {
                            $include=0;
                            $includedata=$data;
                            $includevalue=$svalue;
            ...
            ##var_dump("Include:",$include,$includedata,$includevalue);
