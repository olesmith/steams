...
trait MyMod_Search_Vars
{
    ...
    function MyMod_Search_Var_Add($data)
    {
        ...
        if (!preg_grep('/^'.$data.'$/',$this->SearchVars))
        {
            array_push($this->SearchVars,$data);
        }
    }
    ...
    {
        //????????? Empty sometimes? Every second time... var_dump($_POST);
        ...
        foreach ($this->MyMod_Search_Vars() as $data)
        {
             ...
             if ($this->MyMod_Data_Access($data)>=1)
            {
                ...
                //var_dump($data,$rdata,$value);
                if (!empty($value))
                {
                    if ($value!=" 0")
                    {
                        $where=$this->MyMod_Search_Var_Where($data,$value);
                        if (!empty($where) && $value!=" 0")
                        {
                            if (is_array($where))
                            {
                        ...
                        if (is_array($searchjoined))
                        {
                            foreach ($searchjoined as $rdata)
                            {
                                ...
                                {
                                    $where=$this->MyMod_Search_Var_Where($data,$value,$rdata);
                                    if (is_array($where))
                                    {
                                        $wheres=array_merge($wheres,$where);
                                    }
                        ...
                        $searchvars[ $data ]=$wheres;
                    }
                }
                elseif ($this->MyMod_Search_CGI_Zero_Value($data))
                {
                    $searchvars[ $data ]=
                        "(".
                        join
