...
trait MyMod_Search_CGI
{
    ...
    function MyMod_Search_CGI_Vars_Defined_Has()
    {
        ...
        {
            if ($this->MyMod_Search_CGI_Var_Defined_Has($data))
            {
                return True;
            }
        }
    ...
    //*
    //* function MyMod_Search_CGI_Var_Defined_Has, Parameter list: $data
    //*
    //* Value of include all items cgi field.
    //*

    function MyMod_Search_CGI_Var_Defined_Has($data)
    {
        if ($this->MyMod_Data_Access($data)>=1)
        {
            ...
            if (!empty($value))
            {
                //var_dump($data,$_GET[ $this->ItemData[ $data ][ "GETSearchVarName" ] ]);
                return TRUE;
            }
            ...
            if
                (
                    !empty($this->ItemData[ $data ][ "GETSearchVarName" ])
                    &&
                    isset($_GET[ $this->ItemData[ $data ][ "GETSearchVarName" ] ])
                )
            {
                return True;
            }            
    ...
    function MyMod_Search_CGI_Vars_Defined()
    {
        $datas=array();
        foreach ($this->MyMod_Search_Vars() as $data)
        {
