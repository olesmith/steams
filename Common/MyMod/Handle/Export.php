...
trait MyMod_Handle_Export
{
    ...
    function MyMod_Handle_Export_Do()
    {
        ...
        elseif ($exporttype=="PHP")
        {
            $this->MyMod_Handle_Export_PHP($outdoc,$datas);
        }
        else
        {
    ...
    function MyMod_Handle_Export_PDF($outdoc,$datas)
    {
        ...
        $this->Latex_PDF($texfilename,$latex);
    }
    ...
    function MyMod_Handle_Export_PHP($outdoc,$datas,$sep=";")
    {
        ...
        foreach (array_keys($this->ItemHashes) as $id)
        {
            ...
            {
                $rdata='"'.$data.'"';
                $rdata=str_pad ($rdata,15);
                if ($commentid && $data=="ID") { $rdata="#".$rdata; }
                array_push
                (
                    $php,
                    '      '.$rdata.' => "'.$this->ItemHashes[ $id ][ $data ].'",'
                );
            }
        ...
        $this->MyMod_Doc_Header_Send("php",$outdoc.".php","utf-8");
